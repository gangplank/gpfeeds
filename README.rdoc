...
If you are pushing database changes to Heroku and the `rake db:migrate` command isn't working, you can reset the Postgres database with `heroku pg:reset` then do `heroku run rake db:create` and `heroku run rake db:migrate` again.